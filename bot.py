...
# 🔥 100 PROXY DI PROXYSCRAPE (GIÀ INCLUSA LA LISTA COMPLETA)
PROXYSCRAPE_LIST = [
...
def test_proxy(proxy):
    ...
    try:
        ...
        if r.status_code == 200:
            return {'success': True, 'proxy': proxy, 'time': time.time()-start, 'ip': r.json().get('origin')}
    except:
        pass
